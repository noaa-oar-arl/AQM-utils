...
!  Jianping Huang and Kai Wang   11/22/2022   support AQMv7.0 (ufs-aqm) implementation
!------------------------------------------------------------------------------
program aqm_post_maxi_bias_cor_grib2_1144 
...
!   real(dp), allocatable :: indata1(:,:,:,:,:)	! (COL, ROW, LAY, TSTEP)
   real(dp), allocatable :: indata1(:,:,:)	! (TIME, COL:ny, ROW:nx)
   real(dp), allocatable :: indata2(:,:,:)	! (TIME, COL:ny, ROW:nx)
   real(dp), allocatable :: indata3(:,:,:)	! (TIME, COL:ny, ROW:nx)
...
! read_netcdf_var, in case the very first file is invalid.
   ...
   dims_in3   = dims_in4( (/ 1,2,3 /) )      ! omit vertical LAY dimension
   ...
   if  ( icyc .eq. 6 ) then
      if ( max_proc .eq. 48 ) then
         do i = 1, imax
            do j = 1, jmax
               if ( ichk .eq. 1 ) then
                  bc_op(i,j,1:2)  = indata2(i,j,5:6)  ! using today 00z file
               else
                  bc_op(i,j,1:2)  = indata2(i,j,17:18) ! using previous day 12z file
               endif
               bc_op(i,j,3:nhours) = indata1(i,j,1:46)   ! n_bias_cor_day *24 - 2
            enddo  
         enddo 
      else
         do i = 1, imax
            do j = 1, jmax
               ...
               bc_op(i,j,3:nhours) = indata1(i,j,1:70)   ! n_bias_cor_day *24 - 2
            enddo  
        enddo 
     end if
         ...
         do i = 1, imax
            do j = 1, jmax
               if ( ichk .eq. 1 ) then    !! today's 00Z output is available
                  bc_op(i,j,1:2)  = indata2(i,j,5:6)  ! from 05z and 06z at 00z run
               else
                  bc_op(i,j,1:2)  = indata2(i,j,17:18) ! 17-18Z using previous day 12z file
               endif

               if ( ichk1 .eq. 1 ) then    !! today's 06Z output is available
                  bc_op(i,j,3:8)  = indata3(i,j,1:6)  ! from 01-06 hour at 06z run
               else
                  bc_op(i,j,3:8)  = indata2(i,j,19:24)    ! from 19-24hr at previous day 12z run
               endif
               bc_op(i,j,9:nhours) = indata1(i,j,1:40) ! from 12z run  n_bias_cor_day *24 - 8
            enddo
         enddo
      else
         do i = 1, imax
...
!
               if ( ichk1 .eq. 1 ) then    !! today's 06Z output is available
                  ...
                  bc_op(i,j,3:8)  = indata3(i,j,19:24)    ! from 19-24hr at previous day 12z run
               endif
               bc_op(i,j,9:nhours) = indata1(i,j,1:64) ! from 12z run  n_bias_cor_day *24 - 8
            enddo
         enddo
      endif
   endif
...
end program aqm_post_maxi_bias_cor_grib2_1144
