...
def read_aerosol_species_from_grib2(grib_file_path: str, 
    ...
    try:
        ...
        aerosol_species_info = {
            'dust_bin1': {'parameter': 'du_pm2', 'fullName': 'Dust Dry Particulate matter (fine)'},
            'dust_bin2': {'parameter': 'du_pm20', 'fullName': 'Dust Dry Particulate matter (fine)'},
            'dust_bin3': {'parameter': 'du_pm36', 'fullName': 'Dust Dry Particulate matter (coarse)'},
            'dust_bin4': {'parameter': 'du_pm60', 'fullName': 'Dust Dry Particulate matter (coarse)'},
            'dust_bin5': {'parameter': 'du_pm120', 'fullName': 'Dust Dry Particulate matter (coarse)'},
            'seasalt_bin1': {'parameter': 'ss_pm6', 'fullName': 'Sea Salt Dry Particulate matter (fine)'},
            'seasalt_bin2': {'parameter': 'ss_pm2', 'fullName': 'Sea Salt Dry Particulate matter (fine)'},
            'seasalt_bin3': {'parameter': 'ss_pm10', 'fullName': 'Sea Salt Dry Particulate matter (coarse)'},
            'seasalt_bin4': {'parameter': 'ss_pm30', 'fullName': 'Sea Salt Dry Particulate matter (coarse)'},
            'seasalt_bin5': {'parameter': 'ss_pm100', 'fullName': 'Sea Salt Dry Particulate matter (coarse)'},
            'sulfate': {'parameter': 'so4_pm139', 'fullName': 'Sulphate Dry Particulate matter (fine)'},
            'organic_carbon_hydrophobic': {'parameter': 'omho_pm424', 'fullName': 'Particulate organic matter hydrophobic dry Particulate matter (fine)'},
            'organic_carbon_hydrophilic': {'parameter': 'omhi_pm424', 'fullName': 'Particulate organic matter hydrophilic dry Particulate matter (fine)'},
            'black_carbon_hydrophobic': {'parameter': 'bcho_pm236', 'fullName': 'Black carbon hydrophobic dry Particulate matter (fine)'},
            'black_carbon_hydrophilic': {'parameter': 'bchi_pm236', 'fullName': 'Black carbon hydrophilic dry Particulate matter (fine)'},
        }
        ...
        for attr in ['nx', 'ny', 'la1', 'la2', 'lo1', 'lo2', 'dx', 'dy', 'level', 'shortName', 'parameterNumber']:
                ...
                if param_name == species_info['parameter']:
                    ...
                    try:
                        ...
                        if hasattr(msg, 'data'):
                            ...
                            logger.warning(f"No data attribute found in message")
                            continue
...
def example_usage():
    ...
    try:
        # First, let's just try opening the file and examining its structure
        logger.info("Opening GRIB2 file for initial inspection...")
        grb = grib2io.open(grib_file)
        
        logger.info(f"File contains {len(grb)} messages")
        
        # Get all unique parameter names in the file
        param_names = set()
        for msg in grb:
            param_name = getattr(msg, 'shortName', getattr(msg, 'parameterNumber', 'unknown'))
            param_names.add(param_name)
        
        logger.info(f"Found {len(param_names)} unique parameter names: {sorted(param_names)}")
        
        # Examine first few messages for each parameter type
        param_count = {}
        for i, msg in enumerate(grb[:50]):  # Look at first 50 messages
            param_name = getattr(msg, 'shortName', getattr(msg, 'parameterNumber', 'unknown'))
            if param_name not in param_count:
                param_count[param_name] = 0
                logger.info(f"Message {i}: param={param_name}, fullName={getattr(msg, 'fullName', 'N/A')}")
            param_count[param_name] += 1
            
        logger.info(f"Parameter counts in first 50 messages: {param_count}")
        
        grb.close()
        
        # Now try reading aerosol species with limited levels
        logger.info("Reading aerosol species with limited level range...")
        all_aerosols = read_aerosol_species_from_grib2(grib_file, level_range=(1, 2))
        ...
        print(f"Grid info: {all_aerosols.get('_grid_info', 'Not found')}")
        print(f"Levels: {all_aerosols.get('_levels', 'Not found')}")
