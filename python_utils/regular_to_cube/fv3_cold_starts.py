...
def get_pressure_levels_from_vcoord(vcoord: np.ndarray, 
                                   surface_pressure: Union[float, np.ndarray] = 101325.0) -> np.ndarray:
    """
    ...
    if isinstance(surface_pressure, (int, float)):
        ...
        logger.info(f"Calculated pressure levels from 2D surface pressure field")
...
def read_cubed_sphere_coordinates(tile_file_prefix: str) -> Dict[str, np.ndarray]:
    """
        ...
        try:
            with xr.open_dataset(tile_file) as ds:
                ...
                if lon_var is None:
                    ...
                    raise KeyError(f"Longitude variable not found in tile {tile_num}. "
                                 f"Available variables: {available_vars}")
                
                if lat_var is None:
                    available_vars = list(ds.data_vars.keys()) + list(ds.coords.keys())
                    raise KeyError(f"Latitude variable not found in tile {tile_num}. "
                                 f"Available variables: {available_vars}")
                ...
                if len(geolon.shape) != 2:
                    raise ValueError(f"Expected 2D longitude array for tile {tile_num}, "
                                   f"got shape {geolon.shape}")
                
                if len(geolat.shape) != 2:
                    raise ValueError(f"Expected 2D latitude array for tile {tile_num}, "
                                   f"got shape {geolat.shape}")
                
                if geolon.shape != geolat.shape:
                    raise ValueError(f"Longitude and latitude arrays have different shapes "
    ...
    logger.info(f"Successfully read coordinates from all 6 tiles")
