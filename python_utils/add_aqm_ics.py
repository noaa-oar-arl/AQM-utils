...
def add_aqm_tracers_ics(fv_tracer_file, wrk_ic_file):
    ...
    try:
        ...
        raise Exception("Could NOT find the tracer file:", fv_tracer_file)

    print("========== AQM tracer file:", fv_tracer)

    # Previous day atmosphere file
    try:
        gfs_atm = xr.open_dataset(wrk_ic_file)
    except Exception:
        raise Exception("Could NOT find the IC file:", wrk_ic_file)
    ...
    ds_new2.attrs["source"] = gfs_atm_attr_source
    ...
    try:
        ...
        raise Exception("Error in creating output file tmp1.nc")
...
def parse_args(argv):
    ...
    parser = argparse.ArgumentParser(description="Add air quality tracer variables to ICs.")
...
if __name__ == "__main__":
    ...
    add_aqm_tracers_ics(fv_tracer_file=args.fv_tracer_file, wrk_ic_file=args.wrk_ic_file)
